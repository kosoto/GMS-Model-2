...
public class SearchCommand extends Command{
	...
	public void execute() {
		...
		if(!(request.getParameter("option")==null)) {
			...
			int pageNum = (request.getParameter("pageNum")==null)?1:Integer.parseInt(request.getParameter("pageNum")), count = MemberServiceImpl.getInstance().countMember(), pageSize = 5, blockSize = 5, beginPage = Math.floorDiv(pageNum-1, pageSize)*pageSize+1, endPage = (count>(beginPage+(blockSize-1))*pageSize)?beginPage+(blockSize-1):(int)(Math.ceil(count/(double)pageSize)), beginRow = (pageNum-1)*pageSize+1, endRow= (count>pageNum*pageSize)?pageNum*pageSize:count;
